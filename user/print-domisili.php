...
$query_surat_domisili = "SELECT * FROM surat_domilisi WHERE id_domisili = $id_domisili";
$query_surat_domisili = "SELECT sd.*, w.nama AS nama_warga, w.nik_warga AS nomor_ktp, w.jenis_kelamin, w.tempat_lahir AS tempat_lahir_warga, w.tanggal_lahir AS tanggal_lahir_warga, w.pekerjaan, w.agama, k.alamat_keluarga, k.kepala_keluarga AS keluarga_dari
    FROM surat_domilisi AS sd
    JOIN warga AS w ON sd.id_warga = w.id
    JOIN keluarga AS k ON w.id_keluarga = k.id_keluarga
    WHERE sd.id_domisili = $id_domisili";

$result = mysqli_query($conncet, $query_surat_domisili); // Menggunakan variabel $connect yang benar
...
$filename = preg_replace('/[^a-zA-Z0-9_\-]/', '_', $row['nama_pemohon']) . '_SURAT_DOMISILI.pdf';
...
$pdf->Cell(0, 10, 'SURAT KETERANGAN DOMISILI', 0, false, 'C');
...
$pdf->MultiCell(0, 5, 'Alamat                   : RT.08 Kelurahan Simpang Duren Kecamatan Jambi Luar Kota, Kota Jambi ' . $row['alamat_keluarga'], 0, 'L'); // alamat_keluarga dari tabel keluarga

$pdf->Ln(4); // Spacing
$pdf->MultiCell(0, 6, 'Orang tersebut diatas, adalah benar-benar warga kami dengan berdomisili di RT08 Kelurahan Simpang Duren Kecamatan Jambi Luar Kota, Kota Jambi Provinsi Jambi.', 0, 'L');
$pdf->Ln(4); // Spacing
